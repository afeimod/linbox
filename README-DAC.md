...
- **任意启动顺序均可**：winedac.drv 内置重连看门狗，每 2 秒自动重连
  DAC 窗口并重建桌面槽位 —— 先跑 wine 再点「DAC 显示器」、先点
  「DAC 显示器」再跑 wine、中途关掉 DAC 页面再重开，都会自动接上，
  无需重启 wine（旧版先 wine 后 DAC 会永久黑屏）。
- **终端页自动拉起改为迷你悬浮窗**：在终端跑 wine 命令时，DAC_START
  广播到达后不再强制跳页打断输入 —— 终端上方出现 45% 宽的小窗
  （标题条可点击还原全屏、✕ 关闭）；点悬浮球「DAC 显示器」仍是全屏页。
- **全屏页面新增最小化**：右上角 `[─]`（或返回键）= 回终端、画面在
  悬浮小窗继续运行；`[✕]` = 停止显示。
- **首次建前缀有进度提示**：wrapper 检测到无前缀时打印"正在创建
  wine 前缀（手机上约 1~5 分钟，无输出属正常）"，不再像卡死。
