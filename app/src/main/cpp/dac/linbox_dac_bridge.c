...
static void attach_vk_buffer_direct( struct vk_surface *sf, unsigned int idx, int fence_fd )
{
    ...
    {
        /* 图层定位：桌面坐标 → SurfaceView 坐标（1:1 v1；缩放经 setMatrix） */
        ASurfaceTransaction_setPosition( trans, sf->sc, (float)sf->x, (float)sf->y );
        ...
        if (sf->w && sf->h && (sf->w != sf->width || sf->h != sf->height))
        {
            float sx = (float)sf->w / (float)sf->width;
            float sy = (float)sf->h / (float)sf->height;
            ASurfaceTransaction_setMatrix( trans, sf->sc, sx, 0.0f, 0.0f, sy );
        }
        else
            ASurfaceTransaction_setMatrix( trans, sf->sc, 1.0f, 0.0f, 0.0f, 1.0f );
    }
...
static void on_transaction_complete( void *context, ASurfaceTransactionStats *stats )
{
    ...
    count = ASurfaceTransactionStats_getASurfaceControlCount( stats );
    for (i = 0; i < count; i++)
    {
        ASurfaceControl *c = ASurfaceTransactionStats_getASurfaceControl( stats, i );
        int fd = ASurfaceTransactionStats_getPreviousReleaseFenceFd( stats, c );
        if (fd >= 0) fence_fd = fd;
    }
                                           ...
                                           int32_t dl, int32_t dt, int32_t dr, int32_t db )
{
    ...
    if (dr > dl && db > dt)
    {
        ARect damage = { dl, dt, dr, db };
        ASurfaceDamageRegion dmg = { 1, (ASurfaceRect *)&damage };
        ASurfaceTransaction_setDamageRegion( trans, g_root_sc, &dmg );
    }
...
void Java_com_linbox_apps_dac_DacNative_nativeSendMouse( JNIEnv *env, jclass clazz,
...
void Java_com_linbox_apps_dac_DacNative_nativeSendKey( JNIEnv *env, jclass clazz,
        jboolean down, jint keycode, jint meta_state, jint unicode, jint repeat,
        jint source )
{
    struct dac_input_key k =
    {
        .down = down, .keycode = keycode, .meta_state = meta_state,
        .unicode = unicode, .repeat = repeat, .source = source,
    };
...
{
    struct { uint64_t hwnd; char text[512]; } *tp = (void *)payload;
    if (!g_title_sink || len < 8 || !env) return;
    {
        jstring str = ( *env )->NewStringUTF( env, tp->text );
        if (str)
        {
            jclass cls = ( *env )->GetObjectClass( env, g_title_sink );
            jmethodID mid = ( *env )->GetMethodID( env, cls, "onTitle", "(Ljava/lang/String;)V" );
            if (mid) ( *env )->CallVoidMethod( env, g_title_sink, mid, str );
...
jint Java_com_linbox_apps_dac_DacNative_nativeConnect( JNIEnv *env, jclass clazz,
                                                       jobject surface, jstring socket_path )
{
    ...
    if (!p_recv_handle)
    {
        ...
        if (lib) p_recv_handle = dlsym( lib, "AHardwareBuffer_recvHandleFromUnixSocket" );
        if (!p_recv_handle)
        {
...
void Java_com_linbox_apps_dac_DacNative_nativeDisconnect( JNIEnv *env, jclass clazz )
{
    close_connection();
    if (g_listen_fd >= 0)
    {
...
void Java_com_linbox_apps_dac_DacNative_nativeSetTitleSink( JNIEnv *env, jclass clazz,
                                                            jobject sink )
{
    if (g_title_sink)
    {
        ( *env )->DeleteGlobalRef( env, g_title_sink );
        g_title_sink = NULL;
    }
    if (sink)
    {
        jclass cls = ( *env )->GetObjectClass( env, sink );
        g_title_mid = ( *env )->GetMethodID( env, cls, "onTitle", "(Ljava/lang/String;)V" );
        ( *env )->DeleteLocalRef( env, cls );
        g_title_sink = ( *env )->NewGlobalRef( env, sink );
    }
}
